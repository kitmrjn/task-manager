        ...
        <div class="tk-topnav-right">
                    ...
                    <div class="tk-dropdown-body">
                        <div class="tk-notif-item">
                            <div class="tk-notif-content">
                                <div class="tk-notif-text text-gray-500 text-sm py-2 text-center">
            ...
            <div class="tk-dropdown-wrap">
                <button class="tk-topnav-user" id="profile-btn">
                    ...
                    <svg class="tk-chevron" id="profile-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
                ...
                <div class="tk-dropdown tk-profile-dropdown" id="profile-dropdown">
                    <div class="tk-dropdown-header tk-profile-header">
                        <div class="tk-profile-avatar-lg">
                            {{ strtoupper(substr(Auth::user()->name, 0, 2)) }}
                        </div>
                        <div>
...
<div class="py-10" style="font-family: 'Epilogue', sans-serif;">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
        ...
        @if (session('success'))
            ...
            <div style="display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:1rem;">
                <div>
                    <h1 style="font-size:26px; font-weight:800; color:#0f172a; margin:0 0 .25rem; font-family:'Geist',sans-serif;">User Directory</h1>
                    <p style="font-size:13.5px; color:#94a3b8; margin:0;">Manage employees, roles, and campaign assignments</p>
                </div>
                <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'create-user-modal')"
                        style="background:#0f172a; color:#fff; border:none; border-radius:9px; padding:.6rem 1.25rem;
                               font-family:'Geist',sans-serif; font-size:13.5px; font-weight:700; cursor:pointer;
                               display:inline-flex; align-items:center; gap:.5rem; flex-shrink:0; margin-top:.25rem;">
                            ...
                            <tr class="task-row">
                                ...
                                <td style="text-align:right;overflow:visible;">
                                    <x-dropdown align="right" width="48">
                                        ...
                                        <x-slot name="content">
                                            <button x-on:click="$dispatch('open-modal', 'edit-user-modal'); $dispatch('set-edit-user', {{ json_encode($user) }})" style="width:100%;text-align:left;padding:.5rem 1rem;font-size:13px;border:none;background:transparent;cursor:pointer;">Edit Profile</button>
                                            @if($user->is_active && !$user->email_verified_at)
                                                <form method="POST" action="{{ route('admin.users.resend-invite', $user) }}">
                                                    @csrf <button type="submit" style="width:100%;text-align:left;padding:.5rem 1rem;font-size:13px;color:var(--c-blue);border:none;background:transparent;cursor:pointer;">Resend Invite</button>
                                                </form>
                                            @endif
                                            @if(auth()->id() !== $user->id)
                                                <form method="POST" action="{{ route('admin.users.toggle-status', $user) }}">
                                                    @csrf @method('PATCH')
                                                    <button type="submit" style="width:100%;text-align:left;padding:.5rem 1rem;font-size:13px;color:{{ $user->is_active ? 'var(--c-red)' : '#166534' }};border:none;background:transparent;cursor:pointer;">
...
<x-modal name="create-user-modal" focusable>
    <form method="post" action="{{ route('admin.users.store') }}" enctype="multipart/form-data" class="p-6">
        @csrf
        <h2 class="text-lg font-medium text-gray-900 mb-4" style="font-family:'Geist',sans-serif;">Add New User</h2>
        <div class="mb-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2"><x-input-label value="Full Name"/><x-text-input name="name" type="text" class="mt-1 block w-full" required/></div>
                <div class="col-span-2"><x-input-label value="Email Address"/><x-text-input name="email" type="email" class="mt-1 block w-full" required/></div>
                <div><x-input-label value="Phone"/><x-text-input name="phone" type="text" class="mt-1 block w-full" placeholder="+63 9xx xxx xxxx"/></div>
                <div><x-input-label value="City"/><x-text-input name="city" type="text" class="mt-1 block w-full"/></div>
                <div class="col-span-2"><x-input-label value="Address"/><x-text-input name="address" type="text" class="mt-1 block w-full"/></div>
                <div class="col-span-2"><x-input-label value="Country"/>
        ...
        <div class="mb-4">
            ...
            <div class="grid grid-cols-1 gap-4">
                ...
                <div><x-input-label value="Campaign"/>
                ...
                <div><x-input-label value="Team Leader"/>
...
<x-modal name="edit-user-modal" focusable>
    <div x-data="{ user: {} }" @set-edit-user.window="user = $event.detail">
        <form method="POST" x-bind:action="`/admin/users/${user.id}`" enctype="multipart/form-data" class="p-6">
            @csrf @method('PUT')
            <h2 class="text-lg font-medium text-gray-900 mb-4" style="font-family:'Geist',sans-serif;">Edit User Profile</h2>
            <div class="mb-4">
                <div class="section-eyebrow" style="font-size:11px;font-weight:700;color:var(--c-soft);text-transform:uppercase;letter-spacing:.06em;margin-bottom:.75rem;">Basic Information</div>
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2"><x-input-label value="Full Name"/><x-text-input name="name" type="text" x-model="user.name" class="mt-1 block w-full" required/></div>
            ...
            <div class="mb-4">
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div><x-input-label value="Campaign (Optional)"/>
                    ...
                    <div><x-input-label value="Team Leader (Optional)"/>
    ...
    window.formatIdType = function(type) { return ID_TYPES[type] || type; };
    function getUsedTypes(prefix) {
        const attached = prefix === 'edit' ? [...document.querySelectorAll('#edit-existing-ids input[name="id_type"]')].map(i => i.value) : [];
        const slotTypes = [...document.querySelectorAll(`#${prefix}-id-slots .id-slot`)].map(s => s.querySelector('select')?.value).filter(Boolean);
    ...
    function buildOptions(prefix, selectedType = '') {
        ...
        for (const [val, label] of Object.entries(ID_TYPES)) {
            if (!used.includes(val) || val === selectedType) html += `<option value="${val}" ${val === selectedType ? 'selected' : ''}>${label}</option>`;
        }
        return html;
    }
    function refreshAllDropdowns(prefix) {
        document.querySelectorAll(`#${prefix}-id-slots .id-slot`).forEach(slot => {
            const select = slot.querySelector('select'), current = select.value;
            select.innerHTML = buildOptions(prefix, current); select.value = current;
        });
    }
    function onTypeSelected(selectEl, prefix) {
        const slot = selectEl.closest('.id-slot'), fileWrap = slot.querySelector('.file-wrap'), val = selectEl.value;
        if (val) { fileWrap.style.display = 'flex'; fileWrap.querySelector('input[type="file"]').name = `valid_id_${val}`; }
        else { fileWrap.style.display = 'none'; const fi = fileWrap.querySelector('input[type="file"]'); fi.name = '_pending_id_file'; fi.value = ''; }
        refreshAllDropdowns(prefix); maybeAddNextSlot(prefix);
    }
    function onFileAttached(fileInput, prefix) { if (fileInput.files.length > 0) maybeAddNextSlot(prefix); }
    function maybeAddNextSlot(prefix) {
        const container = document.getElementById(`${prefix}-id-slots`), slots = [...container.querySelectorAll('.id-slot')];
        const usedCount = getUsedTypes(prefix).length, totalTypes = Object.keys(ID_TYPES).length;
        if (usedCount >= totalTypes) return;
        if (slots.length === 0) { addIdSlot(prefix); return; }
        const lastSlot = slots[slots.length - 1], lastSelect = lastSlot.querySelector('select'), lastFile = lastSlot.querySelector('input[type="file"]');
        if (lastSelect?.value && lastFile?.files?.length > 0) addIdSlot(prefix);
    }
    function addIdSlot(prefix) {
        const container = document.getElementById(`${prefix}-id-slots`), usedCount = getUsedTypes(prefix).length, totalTypes = Object.keys(ID_TYPES).length;
        if (usedCount >= totalTypes) return;
        const slotId = `slot-${prefix}-${Date.now()}`, div = document.createElement('div');
        div.className = 'id-slot'; div.id = slotId; div.style.cssText = 'display:flex;flex-direction:column;gap:.4rem;';
        div.innerHTML = `
            <div style="display:flex;align-items:center;gap:.5rem;">
                <select onchange="onTypeSelected(this, '${prefix}')" style="flex:1;border:1px solid var(--c-border);border-radius:6px;padding:.4rem .6rem;font-size:13px;font-family:inherit;background:#fff;">${buildOptions(prefix)}</select>
                <button type="button" onclick="removeIdSlot('${slotId}', '${prefix}')" style="color:var(--c-soft);background:none;border:none;cursor:pointer;font-size:20px;line-height:1;padding:0 .25rem;">&times;</button>
            </div>
            <div class="file-wrap" style="display:none;align-items:center;gap:.5rem;padding:.4rem .75rem;background:#f8fafc;border:1px solid var(--c-border);border-radius:6px;">
                ...
                <input type="file" name="_pending_id_file" accept="image/jpg,image/jpeg,image/png" onchange="onFileAttached(this, '${prefix}')" style="flex:1;font-size:13px;"/>
            </div>`;
        container.appendChild(div);
    }
    function removeIdSlot(slotId, prefix) { document.getElementById(slotId)?.remove(); refreshAllDropdowns(prefix); maybeAddNextSlot(prefix); }
    document.addEventListener('open-modal', e => {
        if (e.detail === 'create-user-modal') { document.getElementById('create-id-slots').innerHTML = ''; addIdSlot('create'); }
        if (e.detail === 'edit-user-modal')   { document.getElementById('edit-id-slots').innerHTML = ''; setTimeout(() => addIdSlot('edit'), 120); }
    });
    </script>
    @endpush
