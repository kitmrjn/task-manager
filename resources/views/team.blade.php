    ...
    <div class="tm-grid" id="memberGrid">
        ...
        <div class="tm-card" data-name="{{ strtolower($member->name) }}" data-id="{{ $member->id }}">
            ...
            <div class="tm-card-top">
                ...
                {{-- RESTORED: Hierarchy check for editing users --}}
                @if(auth()->user()->isAtLeastAdmin() && auth()->user()->roleLevel() >= $member->roleLevel())
            ...
            {{-- RESTORED: Hierarchy check for assigning permissions --}}
            @if(auth()->user()->isAtLeastAdmin() && auth()->id() !== $member->id && auth()->user()->roleLevel() >= $member->roleLevel())
...
async function saveEdit() {
    ...
    if (res.ok) {
        ...
        if (card) {
            ...
            const roleLabel = role === 'team_member' ? 'Team Member' : role === 'manager' ? 'Manager' : (role === 'super_admin' ? 'Super Admin' : 'Admin');
